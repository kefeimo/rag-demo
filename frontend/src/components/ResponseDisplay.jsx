...
function ResponseDisplay({ response, ragSystem }) {
  ...
  return (
    <div className="w-full space-y-4">
      ...
      {/* Low Relevance Warning Banner */}
      {response.relevance_score < 0.65 && (
        <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-r-lg">
          <div className="flex items-start">
            ...
            <div>
              <h4 className="text-sm font-semibold text-yellow-800 mb-1">Low Relevance Score</h4>
              <p className="text-sm text-yellow-700">
                The retrieved documents have low relevance ({(response.relevance_score * 100).toFixed(1)}%) to your query.
        ...
        <div className="flex items-start justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold ${
              ...
              <span>
                {response.relevance_score >= 0.8 ? 'High' : response.relevance_score >= 0.65 ? 'Medium' : 'Low'} Relevance
              </span>
              <span className="ml-0.5">({(response.relevance_score * 100).toFixed(1)}%)</span>
            </div>
            {response.model && (
